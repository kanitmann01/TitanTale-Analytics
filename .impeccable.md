# Impeccable design context (TTL Stats)

Persistent guidance for UI work in `web/`. Updated by the teach-impeccable workflow.
...
| Concern | Where |
|--------|--------|
| CSS variables | `web/app/globals.css` `:root` |
| Tailwind colors / fonts | `web/tailwind.config.ts` |
| Font loading | `web/app/layout.tsx` (Next `next/font/google`) |
| Quality checklist | `.cursor/skills/design-polish/SKILL.md` |

When adding UI, extend existing patterns (panels, section labels, chart tooltip) before inventing new visual systems.
