...
export function getCivMatchups(seasonId: SeasonId): CivMatchup[] {
...
export function getCivMatchupsForCiv(
  seasonId: SeasonId,
  civilization: string,
): CivMatchup[] {
  const allMatchups = getCivMatchups(seasonId);
  return allMatchups.filter(
    (m) =>
      m.civilization1 === civilization || m.civilization2 === civilization,
  );
}
